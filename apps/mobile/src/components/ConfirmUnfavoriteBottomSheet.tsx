...
export const ConfirmUnfavoriteBottomSheet = ({
  visible,
  recipeTitle,
  submitting,
  onConfirm,
    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={onCancel}>
      <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
        <Animated.View style={[styles.backdrop, { opacity: fadeAnim }]} pointerEvents="auto">
          <Pressable style={StyleSheet.absoluteFill} onPress={onCancel} disabled={submitting} />
        </Animated.View>
        ...
        >
          <View style={styles.mainCard}>
            <DSText style={styles.title}>{t.favorites.unfavoriteTitle}</DSText>
            <DSText style={styles.body}>
              {t.favorites.unfavoriteBody(recipeTitle)}
            </DSText>
            ...
            <Pressable
              style={({ pressed }) => [
                ...
                pressed && !submitting && styles.rowPressed,
              ]}
              onPress={onConfirm}
              disabled={submitting}
            >
              <DSIcon name="Heart" size={18} color={COLORS.laranja} strokeWidth={1.75} />
              <DSText style={styles.confirmLabel}>
                {submitting ? t.favorites.unfavoriteSubmitting : t.favorites.unfavoriteConfirm}
              </DSText>
            </Pressable>
          </View>

          <Pressable
            style={({ pressed }) => [styles.cancelCard, pressed && !submitting && styles.cancelCardPressed]}
            onPress={onCancel}
            disabled={submitting}
          >
            <DSText style={styles.cancelLabel}>{t.favorites.unfavoriteCancel}</DSText>
          </Pressable>
        </Animated.View>
      </View>
    </Modal>
