...
export const RecipeDetailScreen = ({
  ...
  return (
    <View style={styles.root}>
        ...
        <View style={[styles.detailBody, { backgroundColor: theme.colors.surface }]}>
          ...
          {(recipe.category || recipe.cuisine) && (
            <Text style={[styles.eyebrow, { color: theme.colors.primary }]}>
              {[recipe.category, recipe.cuisine].filter(Boolean).join(" · ").toUpperCase()}
            </Text>
          )}

          {/* Title */}
          <Text variant="headlineSmall" style={[styles.detailTitle, { color: theme.colors.onSurface }]}>
            {recipe.title}
          </Text>

          {/* Stats */}
          <View style={styles.statsRow}>
            <MetricCard value={recipe.totalTimeMinutes?.toString() ?? "—"} label="Min" />
            <MetricCard value={recipe.servings?.split(" ")[0] ?? "—"} label="Servings" />
            <MetricCard value={recipe.ingredients.length.toString()} label="Items" />
          </View>

          {/* Segmented control — pill within pill */}
          ...
          <View style={styles.sectionHeading}>
            ...
            <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }}>
              {activeTab === "ingredients"
                ? `For ${recipe.servings ?? "your recipe"}`
        ...
        <View style={styles.dialogOverlay}>
          <View style={[styles.dialogBox, { backgroundColor: theme.colors.surface }]}>
            ...
            <Text style={[styles.dialogBody, { color: theme.colors.onSurfaceVariant }]}>
              ...
              <Text style={{ fontWeight: "700", color: theme.colors.onSurface }}>{recipe.title}</Text>
      ...
      {/* AI bar — future feature placeholder */}
      <Surface
        style={[
          styles.aiBar,
          {
